...
equal-area fuel square inside a $3\times3$ rectangular tiling and pure,
un-homogenised cross sections are used --- removing the homogenisation
model from the problem entirely. With pure data, the 1/4-core eigenvalue
...
reference $1.18646$) by $M{=}16$, inside the Monte Carlo standard error, and the
pin-averaged fission-rate maps agree with OpenMC at the sub-1\% RMS level.
...
\item \textbf{Rectangular nodes with per-node widths (no
  ...
  Pure, un-homogenised 7-group cross sections can then be assigned per
...
is a $3\times3$ array of rectangles carrying the pure, un-homogenised
...
\paragraph{Monte Carlo reference, computed directly (OpenMC, v3).}
...
\begin{itemize}
\item $k_{\mathrm{HOM\_ASM}}^{\mathrm{v3}}=1.3336285$ ($\pm3.5$~pcm),
\item $k_{\mathrm{HOM\_CORE}}^{\mathrm{v3}}=1.1868230$ ($\pm3.4$~pcm).
\end{itemize}
...
gives the deviation from the OpenMC v3 homogenised reference in pcm.
...
OpenMC v3 \emph{homogenised} reference: ASM vs.\ $1.3336285$,
...
OpenMC v3 homogenised reference, as a function of angular order $M$
...
$4129.35~\mathrm{cm}^2$ exactly. Pure, un-homogenised 7-group
...
Table~\ref{tab:rect} and Fig.~\ref{fig:mconv} give the $M$ sweep with pure
XS on the single UO$_2$ assembly ($51\times51$ rect. nodes, all edges
...
\begin{table}[h]
\centering
\caption{Rectangular-node OpenPSN (pure material XS). $\Delta$ in pcm
versus the explicitly noted reference. References: the directly computed
OpenMC v3 explicit-geometry (HET) runs, the high-fidelity reference
$1.18646$ of McGraw \emph{et al.}~\cite{mcgraw2014}, nTRACER (SPHINCS),
and the OpenMOC explicit-pin run of this work (0.1~cm tracks, 4 azim,
TY3 polars, circular fuel).}
\label{tab:rect}
\small
\begin{tabular}{lrrr}
\toprule
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.9\textwidth]{c5g7_rect_m_convergence.png}
\caption{Angular convergence of the rectangular-node OpenPSN
(pure material XS) on the C5G7 single UO$_2$ assembly (left) and the
1/4 core (right), against the directly computed OpenMC v3
explicit-geometry references; the green band is one standard error of
the Monte Carlo eigenvalue ($\approx$3.4--3.5~pcm). The 1/4-core result
reaches the reference within one standard error by $M{=}16$.}
\label{fig:mconv}
\end{figure*}

On the 1/4 core the eigenvalue climbs monotonically into the reference:
$-112.1 \rightarrow -38.7 \rightarrow +11.1$~pcm for
$M=8\rightarrow12\rightarrow16$, with $M{=}16$ inside the OpenMC v3
standard error ($\pm3.4$~pcm) and within $+14.7$~pcm of the
high-fidelity reference $1.18646$. This is the cleanest statement of what the method can
do: with the homogenisation model removed, the residual of the nodal PSN
solution against a high-fidelity explicit-geometry transport calculation
is at the level of the Monte Carlo statistics. (The assembly $M$
...
vacuum edges, converges on the reference instead.)

\paragraph{Flux comparison with OpenMC (explicit geometry).}
Figure~\ref{fig:flux} compares the pin-averaged fission-rate maps of the
rectangular-node OpenPSN ($M{=}16$) against the OpenMC v3
explicit-geometry runs on the same problem, for the single UO$_2$
assembly (first row, 17$\times$17 pins) and the 1/4 core (second row,
51$\times$51 pins). Both maps are normalised to unit sum before
comparison so that the difference is purely in the shape.
On the assembly the pin-by-pin relative error has a mean of
$-0.002\%$ and an RMS of $0.070\%$ (max $0.21\%$). On the 1/4 core the
comparison is made assembly by assembly, each normalised to its own sum
before subtraction: this removes the trivial $\nu$-weighting offset
between the two fields (OpenPSN dumps the pin power $\nu\Sigma_f\phi$
while the OpenMC tally is the fission count $\Sigma_f\phi$; the two
differ by a uniform factor per assembly because $\nu_{\mathrm{eff}}$ is
$2.43$ for UO$_2$ and $\approx2.86$ for the MOX, which reproduces the
$+9\%$/$-6\%$ UO$_2$/MOX assembly offset observed before
normalisation). With that definition aligned, the intra-assembly shape
error is a mean of $-0.04\%$ and an RMS of $0.26\%$ (max $0.74\%$) over
all $1056$ fuel pins --- $0.14$--$0.19\%$ RMS in each UO$_2$ assembly
and $0.32\%$ in each MOX assembly, the MOX cells sitting closer to the
stronger local gradients. The two independent solvers agree to within
a few tenths of a percent on the pin-level shape of the full core.

\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{c5g7_rect_flux_compare.png}
\caption{Pin-averaged fission-rate map, rectangular-node OpenPSN
($M{=}16$, pure XS) vs.\ OpenMC v3 (explicit geometry, collision
estimator). One row per problem --- single UO$_2$ assembly
(17$\times$17 pins), then C5G7-2D 1/4 core, fuel region only
(34$\times$34 pins); columns are OpenPSN (pin power $\nu\Sigma_f\phi$),
OpenMC (fission count $\Sigma_f\phi$), and the relative error, both maps
self-normalised per problem (assembly: one normalisation; core: one per
17$\times$17 sub-assembly, which also removes the uniform $\nu$-weighting
offset).}
\label{fig:flux}
\end{figure*}
...
\rowcolor{gray!15}
PSN nodal (BWW pinwise-hom, best pt.) & OpenPSN & 1.1865870 & $+4$ (vs 1.18655)\\
PSN nodal (rect.\ node, $M{=}16$) & OpenPSN & 1.1866067 & $+5$ (vs 1.18655)\\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{OpenPSN in this work.} The two C5G7 studies of
  ...
  $S\times M$ sweep ($M16\_S2$) is $-23.6$~pcm vs.\ the OpenMC v3
  homogenised reference, i.e.\ $+4$~pcm vs.\ 1.18655; with the
  rectangular-node model that removes homogenisation entirely, $M{=}16$
  lands at $+11.1$~pcm vs.\ the OpenMC v3 explicit-geometry reference
  (inside its Monte Carlo standard error) and $+5$~pcm vs.\ 1.18655.
...
$S{=}5$, $M\ge8$, and the best core point is $-23.6$~pcm; the residual is
the homogenisation model itself, and using the self-shielding-aware BWW
recipe, the best core point is $-23.6$~pcm versus the OpenMC v3
homogenised reference (i.e.\ $+4$~pcm versus $1.18655$).
In the \emph{rectangular-node} study the homogenisation model is removed
entirely: with pure 7-group data the 1/4-core eigenvalue converges
...
fission-rate map agrees with OpenMC to
$0.07\%$ RMS on the single assembly and $0.26\%$ intra-assembly RMS on
the core (sub-percent in every assembly, largest in the MOX cells where
the local gradients are steepest).
