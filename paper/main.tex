...
errors: Figure~\ref{fig:bwwflux} compares its pin-averaged fission map
against the OpenMC homogenised-geometry reference on the same BWW
cross sections (the two differ only in the transport method). The
OpenPSN field is the pin power $\nu\Sigma_f\phi$, converted to
$\Sigma_f\phi$ pin by pin with the material fission-weighted $\nu$
($2.441$ UO$_2$, $2.865$--$2.874$ MOX, $2.434$ fission chamber), and
both maps are normalised to unit sum. Over all $1056$ fuel pins the
relative error has a mean of $+0.08\%$, an RMS of $0.17\%$ and a
maximum of $0.38\%$ --- the UO$_2$ assemblies sit uniformly
$+0.21\ldots+0.23\%$ and the MOX assemblies $-0.06\%$, a mild
inter-assembly power-split offset with no pin-level structure. The
homogenised-core eigenvalue therefore sits where the local physics
says it should: the $-23.6$~pcm is a small, smooth, method-level bias,
not a cancellation.

\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{c5g7_bww_flux_compare.png}
\caption{Pin-averaged fission maps $\Sigma_f\phi$ at the best
pinwise-homogenised core point (M16, S=2) vs.\ the OpenMC
homogenised-geometry reference on the same BWW XS, C5G7-2D 1/4 core
(fuel region, 34$\times$34 pins). Left: OpenPSN ($\nu$-corrected);
centre: OpenMC reference (7-group, collision); right: relative error
(fuel pins only, both maps normalised to unit sum).}
\label{fig:bwwflux}
\end{figure*}
...
OpenMC explicit-geometry (fuel circle $+$ water annulus) runs on the same
un-homogenised data, the high-fidelity reference
...
OpenPSN ($M{=}16$, un-homogenised material XS, $\nu$-corrected) vs.\
...
literature. $\Delta$ = $(k-1.18655)\times10^{5}$ in pcm, except where stated
(STELLA vs its PEACH reference 1.18716; OpenPSN vs 1.18646). The original
NEA table prints the relative per-cent error; the pcm values here are
recomputed from the printed $k$.}
\label{tab:lit}
\small
\begin{tabular}{llcc}
\toprule
...
MOC & CRX & 1.18813 & $+158$ \\
MOC (stoch.\ rays) & UNKGRO & 1.18523 & $-132$ \\
\midrule
\multicolumn{4}{l}{\emph{This work}}\\
\rowcolor{gray!15}
PSN nodal (BWW pinwise-hom) & OpenPSN & 1.1865870 & $+4$ (vs 1.18655)\\
...
i.e.\ $+4$~pcm versus $1.18655$. That residual is the homogenisation
model itself, and the best-point pin-power field agrees with the
same-recipe OpenMC map to $0.17\%$ RMS (max $0.38\%$): a smooth
...
fission map $\Sigma_f\phi$ ($\nu$-corrected, same quantity as the Monte
Carlo tally) agrees with OpenMC to
