...
RES=examples/logs/c5g7_sweep.jsonl
WAITPID=45968

# 0) 等当前 M16_S2 进程退出
while kill -0 $WAITPID 2>/dev/null; do sleep 30; done
echo "TS $(date '+%F %T') M16_S2 done, starting M16_S1" >> examples/logs/c5g7_sweep.log

# 1) M16_S1 1/4 芯
python3 -u examples/tools/c5g7_probe.py 16 1 1e-10 > examples/logs/c5g7_M16_S1.log 2>&1
echo "TS $(date '+%F %T') M16_S1 done" >> examples/logs/c5g7_sweep.log

# 2) 单 UO2 组件 M16_S6（17x17 pin, S=6 → 102x102 = 10404 节点）
python3 -u - <<'EOF' > examples/logs/c5g7_asm_M16_S6.log 2>&1
import resource, sys, time
import numpy as np
sys.path.insert(0, '/opt/data/workspace/OpenPSN')
from psn2d import model
from psn2d import __main__ as M
spec = model.load_spec('/opt/data/workspace/OpenPSN/examples/c5g7/study1_homogenised/c5g7_uo2_assembly.yaml')
...
EOF
echo "TS $(date '+%F %T') asm_M16_S6 done, CHAIN COMPLETE" >> examples/logs/c5g7_sweep.log
